...
handCardsContainers.forEach((handCardsContainer) => {
  handCardsContainer.addEventListener("click", (event) => {
    ...
    if (
      event.target.className === `${newGame.currentRound.turn}-holding cards`
    ) {
      event.target.title = "selected";
      event.target.style.boxShadow = "0 0 10px 1px rgb(11, 243, 11)";
    }
  });
});
...
function checkValidAction(action, event) {
  ...
  if (currentRound.type === "first" && currentRound.cardsPlayed === 0) {
    if (action === "pass") {
